...
static void format_time(int seconds, int *hours, int *mins, int *secs)
{
    *hours = seconds / 3600;
...
static void print_formatted_time(WINDOW *win, int y, int x, int seconds) {
    ...
    int hours, mins, secs;
    format_time(seconds, &hours, &mins, &secs);
...
draw_single_frame(win, 0, rows, "GRANNIK | COMPLEX SOFTWARE ECOSYSTEM | FILE NAVIGATOR RAW", 1);
	...
	static void draw_horizontal(WINDOW *win, int y, int start_x, int length, const char *symbol)
	{
	    for (int i = 0; i < length; i++) {
	        mvwprintw(win, y, start_x + i, "%s", symbol);
	    }
	}

	static void draw_vertical(WINDOW *win, int x, int start_y, int length, const char *symbol)
	{
	    for (int i = 0; i < length; i++) {
	        mvwprintw(win, start_y + i, x, "%s", symbol);
	    }
	}
	...
	static void draw_single_frame(WINDOW *win, int start_y, int height, const char *title, int line_type)
{
    ...
    mvwprintw(win, start_y, 0, "%s", top_left);
	    draw_horizontal(win, start_y, 1, actual_width - 2, horizontal);
    ...
    mvwprintw(win, start_y, title_start, "%s %s %s", title_left, title, title_right);
	    draw_vertical(win, 0, start_y + 1, height - 2, vertical);
	    draw_vertical(win, actual_width - 1, start_y + 1, height - 2, vertical);
    mvwprintw(win, start_y + height - 1, 0, "%s", bottom_left);
draw_horizontal(win, start_y + height - 1, 1, actual_width - 2, horizontal);
...
void draw_field_frame(WINDOW *win)
{
    ...
    int field_y = max_y - 3;
    const char *title = system_time_toggle ? "TIME" : "PLAYBACK TIME & PROGRESS";
...
void shutdown_player_thread(PlayerControl *control, pthread_t thread, int *have_player_thread) {
    ...
    if (has_active_file) {
        ...
        if (!joined) {
            ...
            int hours, mins, secs;
            format_time((int)elapsed, &hours, &mins, &secs);
...
player_control.paused ? "PAUSED (fading out)" : "RESUMED (fading in)");
break;
	case 'f':
...
int navigate_dir(const char *target_dir, char *status_buf)
{
    ...
    if (chdir(target_dir) == 0) {
        ...
        snprintf(status_buf, 256, "Access denied or path not exists");
        return -1;
    }
}
